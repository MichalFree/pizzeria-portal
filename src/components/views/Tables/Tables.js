...
const renderActions = (status, table) => {
  switch (status) {
    case 'free':
      return (
        <Button to={`${PUBLIC_URL}/tables/booking/new`} component={NavLink}>Free</Button>
      );
    case 'booked':
      return (
        <Button to={`${PUBLIC_URL}/tables/booking/${table.bookingId}`} component={NavLink}>{table.status}</Button>
      );
    case 'event':
      return (
        <Button to={`${PUBLIC_URL}/tables/events/${table.bookingId}`} component={NavLink}>{table.status}</Button>
      );
    default:
      return null;
  }
...
const Tables = () => (
  <div>
    <div className={styles.component}>
      <h2>Tables</h2>
      <Link to={`${PUBLIC_URL}/tables/booking/new`} activeClassName='active'>New Booking</Link>
      <Link to={`${PUBLIC_URL}/tables/booking/:id`} activeClassName='active'>Booking ID</Link>
      <Link to={`${PUBLIC_URL}/tables/events/new`} activeClassName='active'>New Event</Link>
      <Link to={`${PUBLIC_URL}/tables/events/:id`} activeClassName='active'>Event ID</Link>
    </div>
